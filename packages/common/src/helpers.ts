...
export function getEnumValue(
  ...
  if (
    config.enums &&
    config.enums[type] != null &&
    name in config.enums[type]
  ) {
...
export function getFieldType(
  ...
  if (field.isArray) {
    ...
    if (useImmutable) {
      result = `${new Array(dimension).join(
        'ReadonlyArray<'
      )}${result}${new Array(dimension).join('>')}`;
    } else {
...
export const getType = (convert: Function) => (
  type: Field,
  options: Handlebars.HelperOptions
) => {
  if (!type) {
...
export function convertedType(
  ...
  const realType =
    ...
    `${type.isScalar ? '' : config.interfacePrefix || ''}${
      skipConversion ? baseType : convert(baseType, 'typeNames')
    }`;
